...
last_synced: "2026-08-18T20:53:37+08:00"
status: finalized
---
...
打开一台 8 卡 HGX H100 服务器，里面最贵的部件不是机箱、CPU 或者 800G 网卡，是 8 块巴掌大的金属外壳——封装着 8 张英伟达 H100 SXM5 GPU。
...
从硅原料到出厂卡，价格抽象出**8 倍价差**。这 8 倍里发生了什么、谁拿走了哪一部分，是后续 30 章反复回引的物理基准。

一张 H100 的物理旅程跨 4 个国家、11 个环节、6 个月：
...
这张 BOM 至少有 6 项是第三方拆解或业内估算，公司不披露真实数字，谁说准确谁就是不诚实。目标不是给出一个准确的成本数字，而是建立一个所有人都能对照的基准——每个估算的来源、时点、不确定区间标清楚，让读者知道这个数字从哪里来、有多大水分、应该信几分。
...
到了台积电新竹 Fab 18（5nm / 4nm 主力厂），要走 1,000+ 道工序，跨越数月。光刻、刻蚀、离子注入、沉积、量测、清洗——每一道都重复几十遍。每一道用的设备就来自这五家：[阿斯麦](https://www.asml.com/)（光刻）、Applied Materials（沉积）、Lam Research（刻蚀）、KLA（量测）、东京电子（涂胶 / 显影）。

最关键的一步是 EUV 光刻。EUV 是阿斯麦全球独家——荷兰 Veldhoven 工厂每年出货 40-50 台。[2025 全年交付 48 套](https://www.asml.com/en/investors/financial-results)，其中 46 台为低 NA EUV，单价约 1.83-2.2 亿美元；2 台为高 NA EUV，单价约 3.8 亿美元。
...
> GH100 是 H100 的 GPU 芯片代号，单裸片面积 814 mm²，集成 800 亿晶体管。来源见英伟达 Hopper Architecture In-Depth 技术博客，2022-03。

考虑边缘裸片、缺陷裸片、按性能分箱（binning），单片晶圆实际能用的 H100-class 裸片业内估算 40-50 颗。把晶圆成本摊到裸片上，台积电 4N 单颗 H100 裸片的制造成本300 美元。
...
> SXM5 是英伟达自研的高密度 GPU 模组接口，用于 HGX/DGX 整机；backplane 即承载多张 SXM5 卡的主板基底。

板卡组装包含几道工序：组装散热片、加上 NVLink 4.0 互连芯片、嵌入 VRM、设计散热。

> NVLink 4.0 是英伟达自研的 GPU-GPU 高带宽协议，第 4 代单向带宽 450 GB/s，远高于 PCIe Gen5 的 64 GB/s。VRM（电压调节模块）把 12V 直流降压到 GPU 内部 0.7-1V 工作电压。

8 张 SXM5 卡，加一对 Sapphire Rapids CPU（英特尔 Xeon 第四代）或 Grace CPU（英伟达自研 Arm 服务器 CPU），加配套主板、电源、机箱、网卡，组成一台 HGX H100 整机。整机额定功耗 6-8 kW，重量 100 公斤左右。
...
| 2 | 上游设备摊销 | \$80 | 50-60% | 美 / 荷 / 日 | 阿斯麦 / AMAT / Lam / KLA / TEL |
| 3 | GPU 裸片（台积电 4N） | \$300 | 60-65% | 台湾 | 台积电 |
| 4 | HBM3 stack（5 颗 × 16GB） | \$1,350 | 60-65% | 韩国 | SK 海力士 |
| 5 | CoWoS-S 封装 | \$750 | 业内估算 50-55% | 台湾 | 台积电 |
| 6 | 基板 / 硅中介板 / BT 树脂 | \$80 | 30-40% | 日 / 台 | Ibiden / Unimicron |
| 7 | NVLink 4 交换芯片 | \$120 | 70%+ | 台湾（台积电制造） | 英伟达 |
| 8 | VRM / 电源 / 滤波 | \$100 | 25-30% | 台 / 中 | MPS / Vicor |
| 9 | 散热（vapor chamber） | \$90 | 20-25% | 中 / 台 | AVC / Auras |
| 10 | PCB 板卡 / SXM5 backplane | \$130 | 15-20% | 台 / 中 | 富士康 / Wiwynn 板厂 |
| 11 | 测试 / 组装 / 良率损失 | \$300 | （计入板厂毛利） | 台 / 中 / 捷克 | 富士康 |
| | **物理 BOM 合计** | **~\$3,320** | | | |
| | **英伟达渠道售价（业内估算）** | **\$25,000-30,000** | | | |
| | **8 倍价差归英伟达 + 渠道商** | **~\$24,500** | | | |
...
>
> **毛利率口径**：综合台积电 FY25 整体 59.9%、SK 海力士 FY25 营业利润率 49%、工业富联 FY25 整体毛利率 6.98% / AI 服务器 12-15%、阿斯麦 FY2025 52.8% 等公司财报反推，单环节毛利率属业内估算。
>
> **8 倍价差≠ 英伟达实际毛利率**：本表 87.7% BOM 毛利率指物理成本与渠道售价的倍数。英伟达 FY26Q4 财报口径整体毛利率 GAAP 75.0% / 数据中心业务约 75%+，分母还要扣减 R&D、CUDA 软件、销售管理、保修等，与 BOM 不同口径，不可直接比较。
>
> **整机环节口径**：本表服务器整机环节统一用工业富联（601138，鸿海富士康的 A 股子公司）口径；鸿海母公司 FY25 合并报表 6.15%（含手机代工等低毛利业务），不混用。渠道售价综合 IntuitionLabs 2024 价格指南、戴尔 HGX H100 8-GPU 渠道报价（\$216,000 即单卡 \$27,000）、SemiAnalysis teardown；英伟达不官方披露单卡出厂价。

把这张表纵向看一下，三件事跳出来。

### HBM 比 GPU 裸片贵 4.5 倍

GPU 裸片是英伟达设计、台积电制造，市场把它当作 H100 的灵魂。但物理成本上，5 颗 HBM3 stack 合计 \$1,350，是 GPU 裸片 \$300 的 4.5 倍。
...
HBM 的物理成本因此堆得很高：5 颗 stack × \$200-300 加 TSV 工艺，再叠 12-Hi 堆叠的良率损失。单卡里 HBM 占 BOM 的 41%，是产业链的物理特征，不是英伟达的定价选择。

单卡 BOM 里最值钱的部件，是焊在 GPU 旁边的 5 颗 SK 海力士 HBM3。后面有专章把这件事展开为「HBM 价值远高于 GPU 裸片」这个判断的主战场。
...
台积电 CoWoS 月产能 2024 年仅 35K 片晶圆，2025 年扩到 75K，2026 年目标 130K，是 2024-2025 全产业链最紧的环节。后面有专章把 CoWoS 当作被低估的物理瓶颈展开。

### 8 倍加价的英伟达内部账本

把 BOM 看作 \$3,320，渠道售价看作 \$27,000（取区间中位），差额 \$23,680 是英伟达对客户收取的所有非物理成本。

这笔钱分四块：

- **CUDA 软件税**：开发者用 PyTorch、JAX、TensorFlow 时底层默认调 CUDA，英伟达拿到的是框架层切换成本形成的护城河溢价。详见第 7 章。
- **研发摊销**：英伟达 FY26 R&D 支出 \$18.497B。按 FY26 加速卡出货总量 600-800 万颗摊销，单卡 R&D 摊销约 \$2,300-3,100（R&D 总额覆盖全产品线，按加速卡分母摊销属上限估算）。
- **数据中心整机方案**：英伟达卖的不仅是单卡，还有 NVLink Switch、InfiniBand 网卡（通过收购 Mellanox 获得）、DGX 系统软件、NVAIE 软件订阅。整机方案的 ASP 远高于单卡。
- **渠道分销加客户认证服务**：戴尔、惠普企业（HPE）、超微电脑（Supermicro）、联想这些 OEM 从英伟达拿货后加 5-10% 渠道差价，再卖给最终客户。

> CUDA 是英伟达的 GPU 编程框架，对 AI 训练栈是事实标准。ASP 即 Average Selling Price 平均售价。

\$23,680 里只有一部分进了英伟达口袋。FY26Q4 英伟达整体 GAAP 毛利率 75.0%，扣除运营费用、税之后净利润率约 55-60%。也就是说一张 H100 单卡里，英伟达净拿走约 \$15,000-18,000，其余 \$5,000-8,000 是渠道分销与运营摊销。
...
> Grace 是英伟达自研 Arm CPU，B200 是 Blackwell 代 GPU；GB200 模组把两者打包。B200 单卡 BOM \$6,400、售价 ~\$40,000，毛利率业内估算 84%；GB200 模组 BOM 估算 \$13,500、售价 ~\$65,000，毛利率约 79%。

下面把这张 BOM 中的 GB200 NVL72 整柜对照拉出来。

## 3. GB200 NVL72 整柜的对照旅程

H100 SXM5 是单卡形态，2024-2025 是主力训练 / 推理加速器。英伟达在 2024 年开始把整柜作为更高单位的销售方式。
...
>
> **口径说明**：本表 BOM 指模组级物料成本（裸片 + HBM + CoWoS 等物理材料），与英伟达模组出厂价（72 × ~\$65,000 ≈ \$4.7M）不可混。模组出厂价含英伟达毛利和 R&D 摊销，已包含在整柜出厂价 \$2.8-3.4M 里——英伟达把模组直接组进整柜出货，没有先卖模组再装柜的双重加价。第 9 章 GB200 NVL72 整柜 BOM 推算法（含 NVSwitch tray、互连套件、整柜机架的全口径）给出更宽区间 \$1.0-1.2M，与本表口径一致。

GB200 NVL72 的价值密度更极端——整柜物理 BOM 业内估算 \$1.0-1.2M，出厂 \$2.8-3.4M，加价倍数 2.5-3 倍。

乍看 H100 单卡的 8 倍加价被压缩了，但这是因为整柜方案里英伟达把更多本来由板厂、OEM 渠道吃的部分自己内化了。72 张 GB200 加上 NVLink Switch、整柜级软件栈、液冷设计，英伟达自己做，整柜 ASP 大涨而 BOM 加价倍数缩水。

绝对利润上，英伟达单柜净拿 \$1.5-1.8M（按整柜售价 \$3M × 75% 毛利率，再扣 15-18% 运营费用反推）。

而 72 张 H100 单卡售卖给客户能给英伟达带来的总净利约 \$1.08-1.30M（按单卡净利 \$15K-18K × 72 推算）。

单柜 GB200 NVL72 给英伟达带来的净利明显高于等数量 H100 单卡的合计。整柜方案让英伟达把 OEM 渠道挤出去，自己吃掉了原本由戴尔、惠普企业拿走的渠道毛利。这是英伟达在 Blackwell 时代主动做的产业链垂直整合，第 9 章会展开。
...
整柜对单卡，BOM 结构的差异指向一件事：英伟达在 Blackwell 一代把系统集成度作为新增价值层。单卡售价加价倍数下降但绝对利润上升，整柜级方案把 OEM 渠道毛利吞掉。这是英伟达议价权进一步上行的信号，第 7 章护城河会回到这条线。
...
| 环节 | 单环节毛利率 | 行业平均 PE 估值倍数 | 集中度（CR3 ） |
|---|---:|---:|---:|
| 上游设备（阿斯麦 / AMAT / Lam / KLA / TEL） | 47-60% | 25-35x | 75% |
| 上游硅片（信越 / SUMCO） | 30-35% | 15-20x | 50%+ |
| 晶圆代工（台积电 4N） | 59.9%（FY25 整体）| 22x（TSM） | 90%+（先进节点） |
| HBM（SK 海力士 / 三星 / 美光） | 49%（SK 海力士 FY25 营业利润率）/ 60-65% HBM 单品毛利率 | 12-15x | 100%（三家全占） |
| CoWoS 封装（台积电） | 50-55% | 22x（与台积电合并） | 100%（台积电主导） |
| 加速芯片设计（NVIDIA） | 75%+（NVDA FY26 GAAP）| 30-40x（NVDA） | 90%+ |
| 网络互连（博通 / 美满电子 / Astera） | 50-60% | 20-30x | 70%+ |
| 服务器整机（工业富联 601138 / Wiwynn） | 6.98%（工业富联 FY25 整体）/ 12-15% AI 服务器分品类 | 8-12x | 40% |
| 数据中心 IDC（Equinix / DLR） | 50%（EBITDA Margin，REIT 口径） | EV/EBITDA 22-28x | 30% |
| 电力（PJM / ERCOT 容量市场） | 通胀挂钩，难单独算 | 公用事业 12-18x | 分散 |
| GPU 云 / 超大规模云厂（CoreWeave / AWS） | 60% EBITDA（CoreWeave ）/ 30% 业务部门口径 | 10-50x（差异极大） | 60% |

> 来源：单环节毛利率综合各公司 FY25 一手财报（台积电 FY25 整体 59.9%、SK 海力士 FY25 整体 49%、英伟达 Q4 FY26 GAAP 75.0%、工业富联 601138 FY25 整体 6.98%、阿斯麦 FY2025 52.8%）+ 业内估算（CoWoS 单业务毛利率、HBM 单品毛利率，公司不分品类披露）。服务器整机环节用工业富联 A 股子公司口径（鸿海母公司 Hon Hai 合并报表 FY25 毛利率 6.15%，含手机代工等其他业务）。PE 估值倍数取 2026-05 区间业内常用值，仅作为口径参照，单环节内公司差异大。CR3 = 行业前三家市占率合计，业内估算。
...
U 形曲线对应的产业链含义是：算力链上的高利润玩家集中在四个国家——美国（英伟达、上游设备）、台湾（台积电）、韩国（SK 海力士、三星）、荷兰（ASML）。这四个地方的玩家拿走整条链 70-80% 的利润。
...
| 玩家 | 单卡拿到的金额（USD） | 占售价比 | 单卡净利润 |
|---|---:|---:|---:|
| 信越 / SUMCO（硅片） | ~\$20 | 0.07% | ~\$6-7 |
| 阿斯麦（EUV 摊销 / 折旧） | ~\$30（摊销） | 0.11% | ~\$15 |
| 其他设备（AMAT / Lam / KLA / TEL）| ~\$50（摊销） | 0.19% | ~\$25 |
| 台积电（GPU 裸片 4N 代工） | ~\$300 | 1.11% | ~\$180（按台积电 FY25 整体净利润率 ~40% 估）|
| SK 海力士（HBM3 stack × 5） | ~\$1,350 | 5.00% | ~\$660（按 SK 海力士 FY25 营业利润率 49%）|
| 台积电（CoWoS-S 封装） | ~\$750 | 2.78% | ~\$300（CoWoS 毛利率 50-55%、减运营摊销）|
...
| **英伟达 + 渠道分销** | **~\$23,680** | ** 87.7%** | 英伟达净拿 ~\$15,000-18,000 |

> 来源：BOM 子项分配综合 Silicon Analysts 2026-04 + SemiAnalysis 历年 teardown；台积电 CoWoS 毛利率公司不披露、业内估算 50-55%；SK 海力士 HBM 单卡分到金额按 5 颗 stack × \$200-300/stack 估算；英伟达净利润率按 FY26 GAAP 净利润率（综合 NVDA 各季 10-Q + FY26 全年）反推。所有玩家净利润数字属业内估算，单卡级利润分配不能逐项作为投资测算输入。

这张表的纵向结构很清楚：整条产业链 12 家以上主要上游玩家拿走的 BOM 部分 \$3,320，只占售价 12.3%。剩余 87.7%（约 \$23,680）由英伟达加渠道分销分掉。这是产业链最浓的事实。

进一步拆英伟达加渠道：

- 英伟达净拿约 \$15,000-18,000，按 FY26Q4 GAAP 毛利率 75.0% 加运营费用率 15-18% 反推
- OEM 渠道分销（戴尔、惠普企业、超微电脑等）约 \$2,500-3,500，OEM 加价 10-15%
- 英伟达软件、NVAIE、服务费约 \$1,000-3,000，按企业版订阅价估算
- 运输、关税、保险、渠道库存约 \$1,000-1,500

英伟达自身在单卡上的净利 \$15,000-18,000，链上其他 12 家以上主要上游玩家合计净利约 \$1,300-1,500——英伟达一家拿走的净利润是产业链上游所有玩家净利润合计的 11-13 倍。

这就是「客户集中度才是英伟达估值最脆弱一环」这个判断的物理基础（详细机制留给后面客户集中度与循环交易两章）。英伟达在单卡上的高净利依赖三件事：

- CUDA 软件生态形成的客户切换成本壁垒
- 数据中心方案集成度形成的整柜溢价
- HBM、CoWoS 双瓶颈条件下供给受限带来的定价权

三个条件中任何一个松动，英伟达在单卡上的 \$15,000+ 净利都会被压缩。

这条产业链利润分配的事实回到全书叙事——产业链是腰部加头部环节赚走绝大部分利润、下游脏活与渠道分到残余的结构。几个具体环节后面会单独展开：

- SK 海力士分到 \$660，相对它整体业务的位置权重为什么这么高
- 台积电在 CoWoS 上分到 \$300、GPU 裸片上分到 \$180，合计 \$480 是单卡上仅次于英伟达的金额
- 富士康拿 \$430 上的 \$30-40 净利，对应整柜售价的微薄毛利率
...
| 2. EUV 光刻机 | 荷兰（阿斯麦唯一）| **极低**：无第二家。重建产能 10+ 年 |
| 3. 沉积 / 刻蚀 / 量测设备 | 美国（AMAT / Lam / KLA）| 中等：东京电子部分替代，中国国产有 28nm 替代但 5nm 及以下无替代 |
| 4. GPU 裸片制造 | 台湾（台积电新竹）| **极低**：4N / 5nm 节点三星仅次主，英特尔 Foundry 落后 1-2 代 |
| 5. HBM 制造 | 韩国（SK 海力士 / 三星）+ 美国（美光台湾 fab）| **极低**：三家寡占，CXMT（长鑫存储）落后 6-7 年 |
| 6. CoWoS 封装 | 台湾（台积电龙潭）| **极低**：Amkor / ASE 在补 CoWoS-class 能力，但产能跟进慢 |
| 7. 基板 / 硅中介板 | 日本（Ibiden）+ 台湾（Unimicron）| 中等：日本 ABF 基板 80%+ 集中度 |
| 8. 英伟达加速芯片设计 | 美国（Santa Clara）| **极低**：AMD MI300X 部分替代但 CUDA 生态差距大 |
| 9. 板卡 / 模组组装 | 台湾（富士康嘉义 / Wiwynn）+ 中国大陆 + 捷克 | 高：多家 ODM 能做 |
| 10. 整机集成 | 台湾 + 中国大陆 + 捷克 + 墨西哥 | 高：戴尔 / 惠普企业 / 超微电脑 / Lenovo 多家 |
...
- 英伟达（美国，设计）
- 应用材料（Applied Materials）、Lam、KLA、东京电子（美、日，前道设备）
...
这道工具的物理基础是：6 个关键环节中含美国成分的就有 5 个。阿斯麦虽然是荷兰公司，但设备里包含美国 CYMER 光源加应用材料配件。
...
这是「AI 不是电信泡沫重演」（第 29 章）的一条结构性差异：电信时代的设备制造分散在欧美日中四地；AI 时代的物理瓶颈集中在台湾一地。

### 韩国半导体板块的位置

SK 海力士和三星占了 HBM 与 DRAM 主力 fab。

韩国在 H100 产业链上的贡献度经常被市场低估。只有看过 BOM 表 HBM 占 41% 这个数字之后，才会理解 SK 海力士 FY25 营业利润率为什么能冲到 49%（DRAM 行业历史最高），韩国半导体板块为什么在 2024-2026 这一波 AI 周期里跑出了英伟达之外最强的财务弹性。
...
| 缺口 # | 数据项 | 估算区间 | 估算来源 | 不确定度 |
|---|---|---|---|---:|
| 1 | 英伟达实际 BOM | \$3,000-3,500 | Silicon Analysts / SemiAnalysis / Bernstein 三方拆解 | ±15-20% |
| 2 | 台积电 CoWoS-S 单独毛利率 | 50-55% | 台积电不分品类披露；卖方研报反推 | ±10 个百分点 |
| 3 | HBM3 单 stack 出厂价 | \$200-300/stack | SK 海力士 / 美光 / 三星均不分品类披露 | ±15% |
...
| 6 | 英伟达数据中心业务单卡净利 | \$15,000-18,000 | 由 GAAP 毛利率 + 运营费用率反推 | ±20% |
...
英伟达不披露 BOM，这是 fabless 公司的标准做法。所有第三方拆解都是从公开渠道拼出来的——台积电晶圆成本估算、SK 海力士 HBM 业务推算、富士康整机毛利率反推，再叠工程师团队对良率、按性能分箱的假设。
...
SK 海力士、美光、三星都不分品类披露 HBM 单品价格。\$200-300 / stack 是业内估算。三家厂之间也有价差——SK 海力士对英伟达长合约价高于美光现货价 10-15%。

本书涉及 HBM 单价时一律给区间加业内估算。

**缺口 #4：GB200 NVL72 整柜实际成交价**

\$2.8-3.4M 是市场共识范围。不同客户的实际成交价差很大，超大规模云厂长合约价、二级 GPU 云价、政府客户价各不相同，英伟达和 OEM 不公开分客户报价。
...
英伟达不分单卡披露净利。从 GAAP 毛利率 75% 加运营费用率 15-18% 加单卡售价 \$27,000 反推，单卡净利约 \$15,000-18,000，区间 ±20%。

6 项缺口里，#2 和 #3 在后面 CoWoS 章与 HBM 章单独展开，对这两个数据点做了更细的论证。其余 4 项缺口在涉及到的章节都按本表的区间加业内估算四字标识。

## 8. 怎么用这张地图

BOM 表的每一环都对应后续的一到两章。下面是环节到章节的导航。

| BOM 环节 / 议题 | 对应章节 | 重点 |
|---|---|---|
| 硅片 / 上游设备 | 第 3 章上游设备 | 阿斯麦 / AMAT / Lam / KLA 的隐形垄断结构 |
| GPU 裸片制造（台积电 4N） | 第 4 章晶圆代工 | 台积电工艺溢价 + 三家分蛋糕（台积电 / 三星 / 英特尔） |
| CoWoS 先进封装 | 第 5 章 CoWoS | 被低估的物理瓶颈——产能时间表与外包给 OSAT |
| HBM | 第 6 章 HBM | 「HBM 价值高于 GPU 裸片」的主战场 + 三家厂分化 |
| 英伟达加速芯片设计 | 第 7 章加速芯片 | 75% 毛利率护城河的五层拆解（CUDA + 系统 + 客户认证 + ...） |
| 网络互连（NVLink / InfiniBand / 以太网） | 第 8 章网络与互连 | 博通的隐形 AI 红利 |
| 服务器整机 / SXM 板 | 第 9 章服务器与整柜 | 8-12% 毛利的脏活 + 英伟达整柜价值收编 |
| 数据中心 / 电力 | 第 10 章数据中心与电力 | 钢筋水泥不是瓶颈，电是 |
| 双瓶颈机理（CoWoS + HBM）| 第 11 章双瓶颈 | 为什么是真紧缺 |
...
| 美国 / 中国 / 东亚 / 全球格局 | 第 19-23 章全球格局 | 周期定位与电信对照两条判断的地缘基础 |
...
- 物理 BOM \$3,320，占 12.3%
- 英伟达净拿 \$15,000-18,000，占 55-67%
- 渠道分销 \$5,000-8,000，占 18-29%

链上 12 家以上主要上游玩家拿走的 BOM 部分（除英伟达外），合计净利约 \$1,300-1,500。英伟达一家拿走的净利润是产业链上游所有玩家净利润合计的 11-13 倍。

这条利润分配在后续护城河、客户集中度、vendor financing、周期定位几章会被反复回引——它指向一个反共识判断：算力产业链是高度倾斜的分配，上游集中赚钱，下游分到脏活累活。水涨船高在这里不成立。市场把 AI 火爆等同于全链受益的常识需要修正。
