...
每一个箭头都对应一个真实的物理动作。硅片从信越或 SUMCO 的拉单晶炉里拉出来，进台积电 4N 工艺线走 1,000+ 道工序；HBM3 从 SK 海力士利川 M16 厂堆叠完成；GPU 裸片和 HBM3 在台积电龙潭的 CoWoS-S 封装线汇合；封装好的模组送到富士康嘉义或 Wiwynn 捷克工厂焊到 HGX 整机里；整机最后落到美国超大规模云厂或 CoreWeave 这样的 GPU 云数据中心上架。每个环节下文给出物理参数、估算成本、加价幅度、国别归属。
...
这张 BOM 至少有 6 项是第三方拆解或业内估算，公司不披露真实数字，谁说准确谁就是不诚实。目标不是给出一个准确的成本数字，而是建立一个所有人都能对照的基准——每个估算的来源、时点、不确定区间标清楚，后续每章拎其中一环展开时，所有读者从同一张基准表出发，知道这个数字从哪里来、有多大水分、应该信几分。

对工程师读者，这张表把每天 nvidia-smi 看到的那块卡拆成全产业链坐标；对金融读者，它把财报里的半导体服务器数据中心分类映射到具体物理环节。两类读者从这一章开始有共同语言。

## 核心结论

先看 5 句话——后面所有 BOM 拆解、地理分布、毛利率比较，都是在论证它们：
...
单卡 BOM 里最值钱的部件，是焊在 GPU 旁边的 5 颗 SK 海力士 HBM3。后面有专章把这件事展开为反共识 #2「HBM 价值远高于 GPU 裸片」的主战场。
...
| HBM（SK 海力士 / 三星 / 美光） | 49%（SK 海力士 FY25 营业利润率）/ 60-65% 业内估算 HBM 单品毛利率 | 12-15x | 100%（三家全占） |
...
| 阿斯麦（EUV 摊销 / 折旧） | ~\$30（业内估算摊销） | 0.11% | ~\$15 |
...
这是反共识 #5「客户集中度是 NVDA 估值反身性核心」的物理基础（详细答辩留给后面客户集中度与循环交易两章）。英伟达在单卡上的高净利依赖三件事：
...
SK 海力士、美光、三星都不分品类披露 HBM 单品价格。\$200-300 / stack 是业内估算。三家厂之间也有价差——SK 海力士对英伟达长合约价业内估算高于美光现货价 10-15%。
...
工程师读者建议路线：直接从第 5 章 CoWoS 或第 6 章 HBM 进入（这两章对工程师每天接触的硬件有最直接的工程语言映射），再回到第 7 章英伟达护城河看自己的工程工作所在的产业位置。
...
下一章会把算力抬到经济学层面——它作为生产要素的三重身份（资本、中间品、通用目的技术），与 H100 BOM 这张物理表的关系。然后按产业链上游到下游的顺序，把 BOM 表中的每一环展开。
