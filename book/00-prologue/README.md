...
last_synced: "2026-06-30T02:16:29+08:00"
status: finalized
---
...
这件事第 16 章会拆细分析,第 30 章会作为估值模板的样本反复出现。本章只把它立在桌上——**当 GAAP 净亏和 60% EBITDA 同时存在,传统估值教科书要被重写**。

至此本章三件事摆完:122 天通电 100K H100(物理框架失效)、四角资金回环(合同披露框架失效)、亏着上市的 60% EBITDA(估值框架失效)。
...
AI 算力这一轮如果出现崩盘剧本,形态也会和历史几轮各自不同。第 29 章会用 12 维度对照表把它们逐项比较。本章不下"必然失败"的结论,但把这套结构作为一个周期信号摆在桌上。
...
- **B 因果与机制**(第 11-14 章):双瓶颈(CoWoS + HBM)的物理约束、2027 年拐点的供需测算、瓶颈缓解后的新硬约束
...
- **F 周期与资本**(第 29-32 章):12 维度周期定位、五种估值模板、12 个争议论证、五年前瞻
