...
本章与第 29 章 / 第 30 章的边界——第 29 章给的是整体周期定位 + 三个泡沫顶部预警信号，是「周期处在历史哪个位置」这一个问题；第 30 章给的是按业务模式划分的五种估值模板（设备商 P/E+DCF / GPU 云 EV/EBITDA / 代工利用率敏感性 / 超大规模云分部 EV/EBITDA+PEG / IDC REIT FFO+AFFO），是「具体公司怎么算」的可移植方法论；第 31 章给的是 12 个具体争议各自的可证伪条件，是「判断对错怎么追踪」。三章一气呵成：周期定位（第 29 章）→ 估值工具（第 30 章）→ 判断答辩（第 31 章）。
...
本书「1997 末-1998 中期」定位的证伪条件——使用第 29 章 §29.7 三预警的简化版本（本章不重复第 29 章已展开的三预警机制）：
...
这一章是第七部的收束——周期定位（第 29 章）→ 估值工具（第 30 章）→ 判断答辩（第 31 章）。三章一气呵成。第八部前瞻（第 32 章）将本章的 12 议题判断外推到 5 年视角，给出 2031 的算力地图情景。
