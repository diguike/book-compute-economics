...
last_synced: "2026-08-18T20:56:14+08:00"
status: finalized
---
...
这一章不下任何投资判断。涉及具体公司估值的段落只做产业分析。

## 1. 同一笔 100K H100 支出的三种走法

把问题从抽象拉到具体：现在有一笔 100,000 张 H100 SXM5 的算力支出，按第 1 章的 BOM 粒度推，单卡含服务器、网络、机架的分摊采购成本约 \$32,000，总资本支出约 \$3.2B。再加配套的 data center shell（数据中心物理建筑 + 配电 + 冷却，按 CBRE 2024-2025 基准 \$10-13M/MW，100K H100 对应约 80MW 计 \~\$0.8B），整笔支出大致 \$4.0B 量级。
...
自建模式的代表是 Mag7（Microsoft / Meta / Alphabet / Amazon / Apple / 英伟达 / Tesla，市场上市值最大的七家美股科技公司，本书在第 29 章 / 30 反复引用）里的前四家——MSFT / META / GOOG / AMZN，它们是超大规模云厂主体，自己买 GPU、自己建数据中心、自己用算力训模型 + 卖云服务。
...
| Microsoft | FY24 第二次延（2024 财年） | 6 年 → 6.5 年（部分服务器） | 年化 EPS 增厚低个位数百分比 |
| Alphabet | FY23（2023-01） | 4 年 → 6 年 | FY23 折旧摊销节省 \~\$3.4B |
| Meta | FY23（2023-01） | 4-5 年 → 5 年 | FY23 折旧摊销节省 \~\$1.4B |
| Meta | FY25（2025-01） | 部分服务器 4-5 年 → 5.5 年 | FY25 折旧摊销节省 \$1-2B |
...
这个数字看起来不大，但其反身性效应非常大：Burry 反情景一旦被市场广泛接受，超大规模云厂的 P/E 重新计算就会立刻把 AI 资本支出高峰期的会计利润盲点暴露出来。第 29 章把 Burry 折旧攻击列为 AI 周期「HBM 价值高于 GPU 裸片」这条判断，原因就在这里——它本身不是预测，是把会计选择转化为压力测试的工具。
...
- **物理寿命**：GPU 硬件本身的物理耐用度（按英伟达 H100 的官方寿命设计，正常工作负载下 7-10 年）
...
四家平均自由现金流覆盖度约 2×——经营现金流仍足以覆盖资本支出，当前不依赖再融资。这是第 29 章给出巨头净现金 \$300B + Mag7 周期抗压能力强判断的底层数字。但 Alphabet 与 Meta 的覆盖度已降至 \~1.7×、Amazon 在 FY26 出现单季自由现金流转负——若 AI 周期再走深一年、Meta 资本支出/Revenue 突破 45%，整体覆盖度有进一步下行风险。
...
自建模式涉及估值含义的产业评论，留给后面的三大云算力账本章与估值章展开。这里立这一节的方法论：**EV/EBITDA + 自由现金流覆盖度 + 折旧政策敏感性**三件套，是看超大规模云厂估值的硬框架。
...
这件事的成因和会计选择含义是第 16 章主菜，本章只立一个对照位：**转租模式的会计杠杆是 6 年折旧 + 高杠杆 + 长合同三层叠加**。三层撑的不是同一个数字——长合同（照付不议锁定的收入与利用率）撑的是 60% EBITDA 本身，6 年折旧和高杠杆撑的是折旧、利息之后的账面利润与估值倍数。哪一层动摇，塌的就是哪个数字。

把 CoreWeave 6 年折旧政策跟 Burry 反情景做一次压力测试：

| 折旧期假设 | CoreWeave Q3 2025 折旧摊销（业内估算） | 调整后 EBITDA 利润率 | GAAP 营业利润率 |
|-|-|-|-|
| 当前（6 年，CoreWeave） | \$630M | 61% | +3.8% |
| 5 年（亚马逊 2025 起，服务器） | \~\$756M | 61% | 约 -5% |
| 4 年（Nebius） | \~\$945M | 61% | 约 -19% |
| 3 年（Burry 反情景） | \~\$1,260M | 61% | 约 -42% |

> 测算说明：折旧摊销按 原折旧摊销 × (6/5)、(6/4)、(6/3) 放大，其余收支不变；营业利润率 = （原营业利润 \$51.9M − 新增折旧摊销）/ 单季营收 \$1,364.7M。**调整后 EBITDA 按定义把折旧摊销全额加回，所以四行的 EBITDA 利润率恒为 61%——折旧年限这个旋钮拧的是营业利润，不是 EBITDA**。完整拆解见第 16 章 §2.2。

折旧期从 6 年降到 3 年，CoreWeave Q3 2025 的营业利润率从 +3.8% 掉到 -42%，而 EBITDA 利润率一动不动。这是转租模式估值的核心反身性传导路径——**6 年折旧期是会计选择，市场如果按 EBITDA 给估值，这个选择就完全不进定价；一旦市场改看折旧后的利润，估值倍数就要重算**。
...
按第 1 章 BOM + 第 2 章单位换算 + 本章前面的三种模式会计映射，把每 H100-hour 的等价成本拆成五项：
...
这四件事是后面 GPU 云、超大规模云厂、模型层、估值四章的核心张力，这里先把为什么这四件事是误读的会计基础讲清楚。
...
3. **自建模式的隐性融资融资租赁被市场系统性低估**。MSFT FY25 期末融资租赁余额业内估算已达 \$32B+ 量级（具体行项目未单独披露于 MSFT FY25 10-K，本章使用 SemiAnalysis 2025 综合估算，与后面三大云算力账本章一手对照口径一致）——按经济实质资本支出 = 报表资本支出 + 新增融资租赁重新口径后，MSFT FY25 资本支出 / Revenue 比从 22.9% 调到约 28%，跨越第 29 章周期定位的 25% 警戒线。
...
5. **转租模式的核心结构是 GPU 抵押 + 长合同 + 6 年折旧三层叠加**。CoreWeave 2024 年从 Microsoft 一家拿到 62% 营收 + 总债务 \$14B + 调整后 EBITDA 61% + GAAP 净亏损——四件事在同一张表上的共存只有靠这三层叠加才能解释。三层撑的数字不同：长合同撑 60% EBITDA，6 年折旧和高杠杆撑折旧、利息之后的账面利润与估值倍数。
...
> **免责声明**
> 
> 本章涉及 Microsoft、Alphabet、Meta、Amazon、Oracle、英伟达、CoreWeave、Nebius、Crusoe、Lambda、Digital Realty、OpenAI、xAI、Anthropic 等具体公司的财务披露描述、会计政策评论、商业模式对照与估值方法学讨论，仅为作者基于公开信息（SEC 财报、10-K / 10-Q / 8-K、公司新闻稿、卖方研报、媒体报道）做出的产业研究与会计分析，**不构成任何投资建议**，也不构成对任何公司股价走向的预测、对任何公司估值高低的判断、或建议读者买卖任何证券。市场有风险，投资决策应基于读者自身的独立判断和专业咨询。
> 
