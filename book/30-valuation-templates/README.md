...
last_synced: "2026-08-18T20:59:05+08:00"
status: finalized
---
...
整本书最 heavy 工具箱章。两个模板（NVDA / CoreWeave）做完整建模，占本章 60% 篇幅，给出 中性 / 乐观 / 悲观 三情景假设全套展开；另外三个模板给框架 + 关键 KPI，不展开建模 —— 因为这三类在 SEC / 台积电 IR / NAREIT 公开披露中已经标准化，读者拿到本章给的 KPI 表，可以自己套到任何一家公司上。

完整 Excel 5 套放附录 D-3。

「英伟达估值是不是太高」和「一级 GPU 云估值合不合理」这两个问题，这一章给中性 / 乐观 / 悲观三套模型，**客观评论而非投资建议**。本章严格走 只做产业分析 模式 —— 评论而非建议，不喊价格目标，不做买卖判断。章末必须有完整的 chapter-level disclaimer。  
「客户集中度才是英伟达估值最脆弱一环」这个判断的**主论证**在 30.3 节 —— 量化建模独立成节，给出客户集中度溢价 vs 折价的反身性模型 + 45% 阈值假说。这是本书全章节最关键的一个反共识判断，前面第 7 章（NVDA 五税）、第 29 章（周期 12 维度对照里的「维度 10 客户集中度」）已经多次铺垫，到本章给出**估值含义的量化结论**。
...
NVDA FY26（截至 2026-01-25）刚交完一份难以再被超过的成绩单：全公司营收 \$215.9B（YoY +65%）、数据中心营收 \$193.7B（YoY +68%）、GAAP 毛利率 71.1%、GAAP 净利润 \$120.1B、稀释 EPS \$4.90（YoY +67%）、Q4 单季营收 \$68.1B（QoQ +20%、YoY +73%）、Q4 GAAP 毛利率 75.0%，FY27Q1 营收指引 \$78.0B ±2%。FY26 全年股东返还 \$41.1B（回购 + 分红），剩余回购授权 \$58.5B（来源同上）。FY27 Q1 实际数已于 2026-05-20 披露：营收 \$81.6B（超指引上限）、数据中心 \$75.2B、GAAP 毛利率回到 74.9%、FY27 Q2 指引 \$91.0B ±2%——本节的 中性 / 乐观 / 悲观 DCF 假设以 FY26 为起点，FY27 Q1 实际略好于 base 投影。
...
模型的全部主观性集中在四个变量上：营收增速假设、毛利率假设、资本支出强度假设、WACC（Weighted Average Cost of Capital，加权平均资本成本）假设。本节把这四个变量在 中性 / 乐观 / 悲观 三情景下完整展开。
...
> 来源：FY26 财务实际数来自英伟达 FY26 Q4 新闻稿；卖方 FY27 共识区间取 Bloomberg / Visible Alpha 综合 2026-05；WACC 计算 = 4.3% + 1.4 × 4.5% 风险溢价 = 10.6% 偏高端，市场实际定价 NVDA WACC 业内估算 8.5-9.5%，本表 base 取 9.0%。**业内估算的 中性 / 乐观 / 悲观 全部假设在此明示**。
...
**第二块：训推比例反转**。当前 NVDA 数据中心营收里训练 + 推理大致五五开（业内估算，公司不分类披露）。bear case 假设 2026-2028 推理优化（Mamba / SSM / MoE 稀疏化 / 蒸馏 / FP4 推理 / 推理芯片化 ASIC 化）把推理算力需求大幅压低，推理迁移到 Trainium / TPU v6 / Maia / MTIA / Ascend 等 ASIC 比例上升至 40%+（FY26 ASIC 占比15-20%，参考第 11 章 ASIC 三阶模型）—— 这件事直接打 NVDA 推理增量叙事。
...
一个负责任的估值结论是把三情景的概率权重明示，**本书不给概率权重**，因为概率权重等于隐含的多空判断，违反 只做产业分析 边界。
...
年报口径相对季报口径平滑掉单季波动，拐点对照应以**年报 36% 口径**为基准 —— 这意味着 NVDA 目前**正好处于反身性曲线的峰值溢价位置，距离反转拐点 45% 还有 9 个百分点**（按季报 39% 口径距拐点 6 个百分点）——这是反身性溢价模型的可证伪条件之一（详 §30.3.4）。
...
第 16 章已经把 CoreWeave 2025 年三季度的财报逐行拆开过，这里只列建模要用的数字：
...
这才是 CoreWeave 估值最该用的经济 EBITDA —— **从 61% EBITDA 利润率降到 15.6%**。两者的差额就是设备真实损耗：调整后 EBITDA 把折旧摊销全额加回，这里用 maintenance 资本支出把它按现金口径重新扣回来，得到的才是第 16 章 §2 反复强调的那件事——一门近一半成本是设备损耗的生意，剔除设备损耗后的利润指标不能当盈利能力用。
...
Burry 反情景由两条独立的压力叠加：**一是设备真实损耗按 4 年而非 6 年计——这一条不压 EBITDA（该口径把折旧摊销加回），而是通过 maintenance 资本支出压「真实 EBITDA」；二是租赁单价与利用率同步下行，把 Adj EBITDA 利润率从 61% 压到 38%**。再叠加任一头部客户砍 20% RPO + 利率成本上升。
...
> 假设：Adj EBITDA 利润率从 61% 压到 38%，驱动项是租赁单价下行 + 利用率回落（第 12 章 §3 的 H100 现货 / 合约价剪刀差是这条假设的产业依据），**不是折旧年限**——折旧年限不进 EBITDA 口径（第 16 章 §2.2）。折旧期 6 年 → 4 年的影响走 maintenance 资本支出那一列：本表 maintenance 资本支出占营收比从 2026 年的 60% 升到 2029 年的约 78%，显著高于 base case 的 45%，对应设备按 4 年而非 6 年更新、且营收下行时更新支出刚性。营收侧客户砍单 20% 加权传导。**业内估算的 bear 假设在此明示**。
...
| Crusoe | 1-2 | 公司不披露 | 5 年 | 中度集中 | OpenAI（Stargate Abilene 物业）+ Microsoft | 自建 + 能源套利 + Stargate 物业开发权 |
| Lambda | 0.5-1 | 公司不披露 | 5 年 | 较分散 | 中小客户 + 学术机构 + AI 创业公司 | 客户最分散 + 多 GPU 型号 + 模型微调服务 |
...
| HPC 占营收比 | 51%（FY25 Q4） | HBM 占 \~50%（FY25 Q4 ） | AI 杠杆 |
...
| cap rate | 6-7% | 5.5-6.5% | 8-10%（中国溢价） |
...
五套模板各自的历史估值区间是一面镜子——当前估值落在历史区间的什么位置，就是周期定位那一章的判断在估值层面的具体表现。
...
意味着 **NVDA 不在思科 1999 估值倍数泡沫顶点**，但在 **思科 1999 市值绝对值泡沫顶点的 6-7 倍位置**。这两件事既周期定位的核心判断（周期定位）的两面，也对应第 29 章 §29.7 三个泡沫顶部预警里的市值 / GDP 比那一项。

## 30.9 英伟达的估值是不是太高了

按同一套格式论证：**多空两侧各自的论点、关键变量、结论**。
...
- **本书不给三种情景的概率权重**，因为给了权重就等于给了隐含的多空判断，越过了只做分析的边界。

把本节当作一个假设展开器：每一组假设对应一个隐含估值，自己接受哪一组、对应估值是多少，自己做判断。**本书拒绝给出 NVDA 估值过高 / 过低 / 合理的二元结论**。

## 30.10 一级 GPU 云的估值合理吗

这一节看 CoreWeave 这类一级 GPU 云。同样按多空加关键变量的格式来。
...
这一章给了 5 套模板、十几组中性 / 乐观 / 悲观情景、三十多个估值倍数。所有这些只做分析，不构成投资建议。这条边界值得明确。

**评论与建议的边界**：

| 评论（本章可以做） | 建议（本章不能做） |
|-|-|
| 给中性 / 乐观 / 悲观三套假设 | 给单一目标价 |
...
> 
> 本章财务数据基线截至 2026-05，其中 NVDA FY27 Q1 实际数与当前市场快照已刷新至 2026-06（正文以日期标注）；DCF 中性 / 乐观 / 悲观 模型仍以 FY26 关账数为起点。公司基本面与市场环境可能在阅读时已发生变化。本章中提到的公司股票、估值倍数、目标价等信息均为分析素材，作者不对其准确性、完整性或时效性作任何承诺。本章 中性 / 乐观 / 悲观 三情景假设是作者基于公开信息的推演，**不代表对任何公司未来表现的预测**。
> 
