...
last_synced: "2026-08-18T20:56:30+08:00"
status: finalized
---
...
> 更新（2026-06）：本节用 Q3 2025 这张表做深度解剖样本（理由见 §1）。此后 CoreWeave 又披露了 2026 Q1（2026-05-07），反差只增不减：单季营收跳到 \$2,078M（同比 +112%、环比 +32%），在手订单（Revenue Backlog）从 \$55.6B 暴涨到 \$99.4B（单季新增承诺 >\$40B），2026 全年营收指引 \$12-13B；但同一季 GAAP 净亏损扩大到 \$740M（利息支出 \$536M）、债务总额升到 \$24.9B、单季资本支出 \$7.695B。营收翻倍、合同接近翻倍、债务和利息也同步翻倍——下文用 Q3 2025 拆出来的「高 EBITDA 利润率 + 持续 GAAP 亏损 + 重债务」三件套结构，在 Q1 2026 这张更大的表上完全复制。CoreWeave 2026-03 IPO 后已是上市公司，截至 2026-06-25 市值约 \$53.9B（股价 \$98.76）。
...
把 GPU 云当作一类生意来分析是错的。本章把 CoreWeave 2025 Q3 8-K 当作显微镜，从营收线一直向下走到 GAAP 净亏损，把这中间所有会计与杠杆的取舍逐条拆开到单卡 economics 层面的分析粒度。

CoreWeave 2024 年 [Microsoft](https://www.microsoft.com/) 一家占营收 62%，这个数字在第 30 章会变成客户集中度反身性的核心变量。

「一级 GPU 云能不能持续盈利」这个问题，答案不是能或不能，而是一个条件式：**折旧期假设 × 利用率假设 × 利率水平 × 客户集中度，四个变量决定 CoreWeave 与同业是 utility 类型的长期玩家、还是杠杆套现的短期玩家**。本章把这四个变量的敏感矩阵建出来，给读者一把尺子量自己的判断，估值结论留给第 30 章。
...
| Gross profit | \~470 | \~242 | +94% | 自一手数据 |
| Sales / marketing / G&A | \~280 | \~93 | +201% | |
...
CoreWeave 在 Q3 2025 8-K 披露的在手订单 \$55.6B（截至 2025-09-30，对比上季 \$30.1B 翻倍）。在手订单这个概念在 GPU 云生意里非常关键，它代表已签订合同尚未交付的未来营收。CoreWeave 把合同储备进一步拆成两部分：committed 合同储备（合约期内必须履行的部分）和 total 合同储备（含合约期外可选续约部分），公司未单独公开两者拆分。

在手订单 \$55.6B 对应一家年化营收 \$5-7B 量级的公司，约等于 8-10 年的未来订单。这个数字落到一年期口径上看：CoreWeave 2025 全年营收指引 \$5.05-5.15B，2026 营收指引市场预期 \$12-13B。在手订单的5 年合同叙事，是 CoreWeave 估值最重要的乐观逻辑——这件事在 §4 客户集中度反身性会重新审视。
...
加上 2026-01 [英伟达](https://www.nvidia.com/) 以 \$87.20/share 认购 2293.6 万股、合计 \$2B，CoreWeave 的客户结构是 Microsoft + OpenAI 双核 + 英伟达既是大股东又是供应商又是客户的三角叠加。在手订单 \$55.6B 里 OpenAI 占 \$22.4B、Microsoft 长约市场估 \$10B+（CoreWeave 未单独披露），两家加起来占合同储备 60%+——客户集中度的反身性问题，§4 详谈。
...
**正面（CoreWeave 享受）**：长合约 + 大单价让运营杠杆变好。Microsoft 这样的客户签的是 5-7 年长约 + **照付不议**（照付不议合同：客户按期付款，不管实际使用量是否达到合同约定算力）结构，CoreWeave 不用在销售拓展和客户成功上投入大量 SG&A。Q3 2025 SG&A 业内估算占营收约 20.5%，对比 SaaS 行业典型公司 30-40% 显著偏低。这一项对 EBITDA 利润率的贡献约 +8-10 pp。
...
| 单卡假设 | 6 年（CoreWeave） | 5 年（亚马逊 2025 起，服务器） | 4 年（Nebius） | 3 年（Burry 反情景） |
|-|-|-|-|-|
| H100 整机采购成本（含 server） | \$32,000 | \$32,000 | \$32,000 | \$32,000 |
| 残值假设 | \$4,000（12.5%） | \$4,000 | \$4,000 | \$4,000 |
| 年折旧 | \$4,667 | \$5,600 | \$7,000 | \$9,333 |
| 月折旧 | \$389 | \$467 | \$583 | \$778 |

> 假设：\$32,000 是 H100 SXM5 单卡含服务器、网络、机架的**分摊采购成本**（per GPU 口径，8 卡 DGX H100 整机约 \$256K）；按 per GPU 而非 per node 折算更便于横向比较。四列统一按 12.5% 残值测算，便于横向对比（旧版三列曾分别用 12.5% / 10% / 零残值，口径不一致，已更正）。残值假设按公司方常用口径，CoreWeave / Nebius 不单独披露。各家折旧年限出处见 §3.1 对照表。

折旧期从 6 年改到 4 年，单卡年成本立刻从 \$4,667 涨到 \$7,000——增加 50%。这个增加全部直接打在 Cost of Revenue 那一行，从营收里扣掉。

把这个杠杆乘到 Q3 2025 的折旧摊销 \$630M 上。先讲清一件容易搞反的事：**调整后 EBITDA 按定义把折旧摊销全额加回，所以改折旧年限，调整后 EBITDA 与它的利润率一分都不变**。被改动的是折旧之后的 GAAP 营业利润。

| 情景 | Q3 2025 折旧摊销 | 调整后 EBITDA | 调整后 EBITDA 利润率 | GAAP 营业利润 | GAAP 营业利润率 |
|-|-|-|-|-|-|
| 当前（6 年折旧） | 630 | 838 | 61% | 51.9 | +3.8% |
| 5 年折旧 | \~756 | 838 | 61% | \~-74 | 约 -5% |
| 4 年折旧 | \~945 | 838 | 61% | \~-263 | 约 -19% |
| 3 年折旧 | \~1,260 | 838 | 61% | \~-578 | 约 -42% |

> 测算说明：把折旧摊销按 原折旧摊销 × (6/5)、(6/4)、(6/3) 比例放大，其余收支不变；营业利润 = 原营业利润 \$51.9M 减去新增折旧摊销，除以单季营收 \$1,364.7M 得营业利润率。3 年情景与 Burry 在 Cassandra Unchained 主张的短折旧期反情景一致。
> 
> 旧版本表曾把折旧年限变化的影响记在「调整后 EBITDA 利润率」上（61% → 38% → 15%），与该指标的定义不符，已更正为营业利润率口径；序章 §3 用的是同一套口径。

```mermaid
xychart-beta
    title "四种折旧期下 CoreWeave Q3 2025 单季折旧摊销（USD M）"
    x-axis ["6 年（CoreWeave）", "5 年（亚马逊）", "4 年（Nebius）", "3 年（Burry 反情景）"]
    y-axis "单季折旧摊销（USD M）" 0 --> 1400
    bar [630, 756, 945, 1260]
```

> 折旧年限这个旋钮改变的是上面这根柱子的高度——它整根被加回到调整后 EBITDA 里，所以 EBITDA 利润率纹丝不动 61%；而它整根都要从营业利润里扣掉，所以营业利润率从 +3.8% 掉到 -42%。这正是 Michael Burry 2025-11 公开攻击超大规模云厂折旧政策的核心依据（详见 §5）：一门近一半成本是折旧摊销的生意，用一个把折旧摊销剔除在外的指标去讲盈利能力。

这是 CoreWeave 故事里第一个折旧期是估值生死线的具象数字。**6 年 vs 4 年这一个会计选择，单季营业利润相差 \$315M，等于 CoreWeave 年化 \$1.26B 的会计盈利差**。
...
**折旧年限不出现在这张拆解表里**。调整后 EBITDA 把折旧摊销全额加回，会计选择再怎么调都不影响这个口径——它影响的是营业利润率（§2.2 那张表，6 年 vs 4 年相差 23 pp）。能解释 EBITDA 利润率高低的，只有现金成本结构：

| 来源 | 对调整后 EBITDA 利润率贡献 |
|-|-|
| 客户集中度 → SG&A 低 | +8-10 pp |
| 高速增长摊薄固定成本 | +5-8 pp |
| 其余（运营效率、规模化议价、自建 vs 租赁带来的成本归属差异） | +5-10 pp |
| **合计** | **+18-28 pp** |

> 业内估算，本书基于一手财务披露 + 多份卖方研报推演。各分项区间有部分重叠（客户集中度低 SG&A 与高速增长摊薄固定成本两条都涉及单位营收上的费用率），因此按区间而非点估计给出。

这组区间可以用同业的一手数字校验：Nebius 2026 Q1 调整后 EBITDA 利润率 32.6%（\$130M / \$399M），CoreWeave 同期 61%，两者相差 28 pp——与上表三项合计的上沿吻合。**这 28 pp 里没有一分来自折旧年限**，全部来自现金成本结构：客户集中度决定 SG&A 强度、增长速度决定固定成本摊薄、自建还是租赁决定同一笔算力成本落在折旧还是落在租金（落在租金就会压低 EBITDA）。

所以 CoreWeave 的 61% 要分两句话说：**这个数字本身是真实的现金毛利能力，可持续性取决于客户结构和增长速度；但它不能被当作盈利能力的证据**——因为这门生意近一半的成本是折旧摊销，而 EBITDA 恰好把这一项排除在外。Burry 在 2025-11 把这件事搬到公开市场，攻击的正是后半句，§5 详谈。
...
**第二**，CoreWeave 6 年折旧和 Nebius 4 年折旧是 GPU 云行业里折旧期最长 vs 最短的两个极端。这个差异体现在两家的营业利润和净利润上，不体现在 EBITDA 上：Nebius 2026 Q1 调整后 EBITDA \$130M / 营收 \$399M = EBITDA 利润率 32.6%，CoreWeave 同期 61%，这 28 pp 的差距来自现金成本结构（SG&A 强度、规模摊薄、自建 vs 租赁），与折旧年限无关（§2.3 已拆）。折旧年限的差异要到营业利润那一行才看得见——也正因为如此，只看 EBITDA 的投资者会系统性地低估折旧期选择的影响。
...
**当前 CoreWeave 大致位于 [6 年, 8%, 85%] 单元格附近**（业内估算债务利率 8.9%、利用率 85% 综合）。对应单卡月净利润 \~\$1,295。
...
CoreWeave 在 S-1 里把客户集中度列为风险因素第一条，但是市场长期把这件事看成双方都签了长合约，问题不大。本节论证客户集中度不仅是风险，更是估值反身性的核心——完整模型在第 30 章。
...
| 2026-Q1 RPO | 在手订单 \$99.4B；10 家客户合同金额 ≥ \$1B/家 | CoreWeave Q1 2026 8-K |

> 来源：CoreWeave S-1（2025-03，SEC EDGAR 1769628）、SEC DRSLTR（公开信件 2025）、Q3 2025 8-K、Q1 2026 8-K。
> 
> **RPO（Remaining Performance Obligation，美国 GAAP ASC 606 下的剩余履约义务）**：已签合同但未交付收入总额，是会计口径概念。本章 RPO 用的是 ASC 606 严格定义，与 CoreWeave 管理层口径的在手订单不完全一致——在手订单含合同期外的可选续约部分，RPO 不含；二者数字接近但不可混用。本表 2026-Q1 RPO \$99.4B 是 CoreWeave Q1 2026 8-K 公司管理层口径（含可选续约部分），与会计 ASC 606 RPO 数字略有差异（公司未单独披露差额）。
...
这件事在数据缺口 #7（CoreWeave 与 OpenAI 协议的现金流分布）里被标为业内估算——CoreWeave 与 OpenAI 都未公开年度现金流分布。5 年合同的年度现金流分布在 \$2-7B 不等（取决于 OpenAI 的算力消费曲线）。
...
把客户集中度的反身性变成一组可量化的数字（完整模型在第 30 章）：
...
从 \$120B 到 \$60B 的估值半价区间，反身性触发器只需要 Microsoft 的一个内部决策——CoreWeave 估值的脆弱性远高于其合同储备数字暗示的稳定性，根源就在这里。本书「客户集中度是估值最脆弱一环」这条判断「客户集中度是估值反身性核心」的机制埋在这里，结论留给第 30 章。

CoreWeave 商业模型最特别的一环不在损益表，而在资产负债表——用已签客户合同作为现金流凭证，把 GPU 资产抵押给银团 + 私募信贷拿融资，再用融资去买更多 GPU：

```mermaid
graph LR
    A[Microsoft / OpenAI / Meta<br/>5-7 年照付不议长合约] --> B[在手订单现金流可见性<br/>$55.6B]
...
> 这套结构能成立的前提是：客户长合约真实兑现 + GPU 二手价残值假设成立 + 利率上行不超过模型容忍区间。三个前提任一动摇，回环会从自我强化转为自我崩塌。这是第 29 章 §29.9 把循环交易列入周期顶部信号的微观基础。
...
1. **长合约支撑长折旧**：照付不议 5-7 年长合约让 GPU 的现金流可见性远超过 12-24 个月。在手订单 \$55.6B 是 5-7 年的合同收入锁定。
...
把这三个变量摆在一起看，**Burry 那个 3 年折旧的反情景是激进的，但市场用 4 年折旧（Nebius 的做e）重新给 CoreWeave 定价的概率不低**。这是「客户集中度是估值最脆弱一环」这条判断「客户集中度反身性」之外，CoreWeave 估值波动的另一个核心传导路径。
...
> 业内估算综合 Morgan Stanley / Mizuho 2025 GPU 云覆盖研报 + 各家公开营收口径。[Vultr](https://www.vultr.com/) 等长尾玩家合计约 17%。CoreWeave 单家占独立 GPU 云市场过半——这是「一级 GPU 云能否持续盈利」这个问题的微观底盘。
...
| 最新一期营收（USD M） | 1,365（Q3 2025） | 399（Q1 2026） | \~200/季 | \~150/季 |
| YoY 营收增速 | +134% | +684% | n.a.（未披露） | n.a.（未披露） |
| 调整后 EBITDA 利润率 | 61% | 32.6%（AI 业务） | n.a. | n.a. |
| GAAP Net margin | -8% | n.a.（受一次性收益干扰） | n.a. | n.a. |
| GPU 折旧年限 | 6 | 4 | 5 | 5 |
| 总债务（USD B） | \~14 | \~0.5（净现金） | \~15（项目融资） | \~3 |
| 在手订单（USD B） | 55.6（2025-09） | n.a. AI 年化经常性收入 1.9 | n.a. | n.a. |
| Top 客户集中度 | Microsoft 62%（2024） | 多元化 + OpenAI / Google 拓展 | 单一锚客户（Oracle 15 年长租 + OpenAI） | Microsoft + 长尾 |
| 估值 | 市值 \~\$70-100B（2026-05 范围） | 市值 \~\$25-30B（2026-05） | \$15B（D 轮后 + Stargate） | \$2.5-4B（D 轮 2025-02，来源分歧） |
...
|-|-|-|-|-|
| 主估值锚 | 在手订单 \$55.6B + EBITDA 61% | AI 年化经常性收入 + 全球化 | Stargate 项目 + Oracle 锚租 | 长尾客户 + 多元化 |
...
1. **CoreWeave Q3 2025 8-K 是观察 GPU 云商业模式最干净的样本**。营收 \$1,365M、调整后 EBITDA \$838M（61% margin）、GAAP 净亏损 \$110M、利息支出 \$310M、债务总额 \$14B、在手订单 \$55.6B——单张 8-K 同时包含高 EBITDA 和高杠杆两种叙事。
2. **61% 调整后 EBITDA 利润率衡量的是现金毛利能力，不是盈利能力**。它由现金成本结构撑起来（客户集中度贡献 8-10 pp、增长摊薄 5-8 pp、其余 5-10 pp），与折旧年限无关——调整后 EBITDA 把折旧摊销全额加回。折旧年限这个旋钮打在营业利润率上：6 年 vs 4 年相差 23 pp，6 年 +3.8% 对 3 年 -42%。在一门近一半成本是折旧摊销的生意里，用 EBITDA 讲盈利能力，等于把最大的一项成本排除在题面之外。
...
7. **GPU 云生意的稳态盈利区间是 -25% 到 +15% 自由现金流 margin**。这个跨度反映了对折旧 / 利率 / 利用率三个变量的极高敏感性。「一级 GPU 云能否持续盈利」的答案是条件式：四个变量决定，没有单一答案。
...
四个问题的答案在第 30 章重新出现，构成 CoreWeave / Nebius 完整 DCF 估值模板的核心输入。在那之前，本章这份 60% EBITDA 拆解，是市场上对 CoreWeave 商业模式最细粒度的公开分析之一。
