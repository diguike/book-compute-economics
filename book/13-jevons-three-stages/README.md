...
last_synced: "2026-08-18T20:55:51+08:00"
status: finalized
---
...
[OpenAI](https://openai.com/) 的 o1（2024-09）把推理时计算（推理时计算）作为模型能力的独立维度，单次 query 消耗的算力从 GPT-4 时代的几千个 FLOP-单 token 量级拉到 o3 / Claude 4 thinking 时代的几十倍量级（业内估算，OpenAI / Anthropic 均不披露 per-query 推理计算）。[Anthropic](https://www.anthropic.com/) 的 Constitutional AI（2022-12 论文）与后续 RLHF / DPO 工程把后训练算力从主训练后的小修小补变成和主训练同量级的独立环节。[DeepSeek](https://www.deepseek.com/) R1（2025-01）在论文里直接披露 RL 后训练阶段消耗占总训练 compute 的相当一部分。二段分类塞不下这些新结构。
...
timeline
    ...
    section 2026 三段定型
        o3 + Claude 4 + 智能体量产 : 前训练 35% / 后训练 25% / 推理 40%
    section 2027 拐点
        推理首次过 50% : 前训练 28% / 后训练 22% / 推理 50%
```

算力需求可以拆成三段：**前训练**（pre-training，传统的下一词预测主训练）、**后训练**（post-training，RLHF / DPO / Constitutional AI / RL-from-verifiable-rewards / o1-style 推理时训练扩展）、**推理服务**（推理，终端用户 token 产生）。

拆成三段之后，市场上吵得最凶的那个问题——「未来训练和推理各占多少算力」——立刻露出了它的真面目：**争的不是事实，是后训练该算在哪一边。** McKinsey 把后训练大部分记进推理，Epoch AI 倾向记进训练，同一组事实就算出了 1:3 和 1:1 两个结论。把口径并排摆出来，这场争论大半是表观问题。

真正有实质分歧的是另一个问题：**推理单价一路暴跌，算力总支出到底是跟着跌，还是反而涨？** 这里给一个明确判断——未来 18 到 36 个月，杰文斯效应主导，企业的 AI 预算还没有成为天花板。

但杰文斯不是物理定律，是一条有前提的经济学规律。它成立需要三个条件同时满足，后文逐条给证据：

| 条件 | 含义 | 用什么验证 |
|---|---|---|
| 使用门槛低、应用远未饱和 | 降价能真的换来新用量 | DeepSeek R1 之后英伟达数据中心营收曲线、Cursor / Replit 等编码助手的 token 消耗爆发、Anthropic 收入增长 |
| 算力不是下游的瓶颈 | 算力便宜了，别处不会立刻卡住 | 反例是电力硬约束和客户集中度 |
| 算力与其他投入是互补而非替代 | 用得越多，配套投入也越多 | OpenAI 从 GPT-4 到 o3 的能力阶梯 |

杰文斯弹性的传导链可以画成下面这张图：

```mermaid
graph LR
    A[模型架构 + 硬件迭代] --> B[单 token 成本 ↓ 95%]
    B --> C[使用门槛降低]
    C --> D[新应用打开<br/>agent / 检索增强 / 长链推理]
...
把这两件事在 18-36 个月的窗口里压实之后，下一个问题是 2027 年之后会怎样。这里给的判断是：**2027 年之前，训练（前训练加后训练）仍然占主导；2027 年中之后，推理算力占比才第一次跨过 50%。**

这和市场主流的说法拉开了距离。券商在 2024-2025 年反复讲「未来 90% 的算力会跑推理」，这里的回答是——迁移确实在发生，但不会发生在 2026 年，会发生在 2027 之后。章末给出可以证伪它的四个信号，每个都配基线、阈值、时间窗和监测来源。

还有一条历史参照值得先摆出来：**新技术的投入和它的产出之间，隔着很长一段时滞。**

1985 到 1995 年，美国大企业的 IT 投资爆发式增长，但全要素生产率（TFP，衡量单位投入产出效率的宏观指标）的回升要等到 1995-2004 年才显现。经济学家 Brynjolfsson 和 Hitt 把这段十年的滞后总结成三个阶段：先投钱买设备，再花几年重组流程，最后生产率才反映出来。因为曲线先向下再向上，这个形状被叫作 J 曲线。

照这个类比，AI 周期从 2022 年底的 ChatGPT 起步、资本支出拐点在 2024 年，那么它对宏观生产率的可观测贡献要等到 2032-2037 年（宏观部分另有专章）。这里只取其中一个推论：**2024-2026 年还在 J 曲线的底部**。企业 IT 的 AI 渗透率从 2024 年的约 3% 走到 2026 年的约 8%，预算的扩张才刚开始，远没到耗尽的拐点。

> 渗透率背后的支出数据：企业 AI 支出从 2023 年的 \$2.3B 涨到 2024 年的 \$13.8B（Menlo Ventures《State of Generative AI in the Enterprise》2024-11）。\$13.8B 是含基础设施、平台、应用的全口径，其中纯应用层约 \$4.6B。

> 三处口径需要先说清楚。**后训练算力占比**是 Epoch AI 估算、DeepSeek R1 论文披露、OpenAI 与 Anthropic 公开访谈三者的综合反推，各家口径相差 20-40%。**企业 AI 渗透率**来自 Menlo Ventures 与 Gartner 的问卷调研，不是财务数据，应答偏差可能让数字偏高 10-20%。**单 token 价格跌 99%** 这个说法取决于怎么比——同一个模型从首发到现在的降幅，和不同代际之间同等能力档的对比，两种口径差约 50%。
...
**第一个现象是后训练成本的指数级膨胀**。Anthropic 在 2022-12 论文里提出 Constitutional AI，用 AI 反馈 + 宪法原则替代纯人类 RLHF 反馈。这套方法的算力消耗在 Claude 2 时代（2023）还是主训练的一个零头；到 Claude 3.5 Sonnet（2024-06）已经被 Dario Amodei 在公开访谈里描述为后训练 is becoming a larger and larger portion of total compute。OpenAI 的 RLHF 工程从 InstructGPT（2022-01）到 ChatGPT（2022-11）到 GPT-4 RLHF（2023）演进同样路径——后训练阶段从主训练的微调变成和主训练同量级的工程。

**第二个现象是 o1 的推理时训练（推理-time 训练 / 推理时计算）打破了训练 vs 推理的物理界限**。OpenAI 在 2024-09 发布 o1，核心创新是模型在回答用户问题前先生成一段思考链（chain-of-thought），思考链本身消耗大量推理 compute。OpenAI 在博客里直接画出一张图——模型性能随训练时 compute 和推理时 compute 两个维度同时 scaling。当推理这一步本身就在做 RL-style 的搜索 + reranking，推理和训练的物理区分开始变得模糊。o1 单次复杂数学问题的推理 compute 业内估算是 GPT-4o 的 50-100 倍。
...
把训练拆开之后会发现，前训练和后训练之间的工程差别，比训练和推理之间还大。把它们都塞在训练里讨论，直接造成了「训推比例」这场争论里的口径混乱。
...
把三段各自的算力占比量化出来，是讨论训推比例的前提。三段的算力占比在 2022-2026 经历了三次结构性切换。

**2022 年（GPT-3.5 / Claude 1 时代）的三段占比是 80 / 5 / 15**（业内估算综合 Epoch AI 2023-2024 数据 + OpenAI 公开访谈）。前训练算力是绝对主菜——GPT-3.5 训练消耗约 3e23 FLOP，InstructGPT-style RLHF 的算力消耗在主训练的 5% 以内。推理算力在 ChatGPT 发布前几乎可以忽略不计（OpenAI API 调用量 2022-Q4 月活 < 1000 万开发者，来源：The Information 2023-03 报道）；ChatGPT 2022-11 发布后两个月达到 1 亿月活，推理算力开始放量。

**2023-2024 年（GPT-4 / Claude 2-3 时代）的三段占比演变到 60 / 10 / 30**（业内估算）。前训练算力绝对值在涨——GPT-4 训练消耗 2e25 FLOP，比 GPT-3.5 高约 70 倍。但推理算力增长更快——ChatGPT 周活在 2023-2024 反复破历史新高，OpenAI API 调用量年增长率 > 5×（综合 OpenAI 公开访谈与 The Information 报道）。Anthropic 的 Constitutional AI 工程让后训练算力占比从 5% 升到 10%——RLHF 多轮迭代 + AI 反馈训练的算力消耗加总，约等于主训练的 15-20%。

**2024-2026 年（GPT-4o / o1 / o3 / Claude 3.5-4 / DeepSeek V3-R1 时代）的三段占比演变到 35 / 25 / 40**（业内估算）。这是过去四年最大的一次结构切换。前训练绝对值仍在涨（Epoch AI 2024-05 测算前沿模型训练算力年化增长 4-5×；来源：Epoch AI "Training Compute Grows 4-5x per Year"），但占比在下降。后训练算力占比从 10% 跳到 25%——核心驱动是 o1-style 推理时训练（OpenAI / Anthropic / Google 都跟进）+ DeepSeek R1 演示的 RLVR 路径。推理算力占比从 30% 跳到 40%——驱动是 ChatGPT 周活 2026-Q1 达到 7-10 亿+ 企业 AI 调用量从 2023 的 \$23 亿涨到 2024 的 \$138 亿（6× 年增长，来源：Menlo Ventures "State of Generative AI in the Enterprise" 2024-11）+ 编码助手 / 客服 agent / 多模态调用的 token 量爆发。
...
| 2026| 35% | 25% | 40% | 250× | o3 + Claude 4 thinking + 智能体量产 |
| 2027（预测）| 28% | 22% | 50% | 500× | 推理算力第一次跨过 50% 阈值 |

> 来源：三段占比为业内估算综合 Epoch AI 训练算力数据库 2024-2025 + DeepSeek V3/R1 论文 2024-12/2025-01 + OpenAI o1/o3 公开博客 + Anthropic Dario Amodei 公开访谈 2024-2025 + Menlo Ventures 企业 AI 报告 2024。总算力规模指数以 2022 年全球前沿模型训练 + 推理算力总和为 1，按英伟达数据中心营收增长 + ASIC 装机增长综合反推。2026 / 2027 行为本章预测，2027 行的推理首次跨过 50%是本章对「推理主导要到 2027 之后」这条判断 的可证伪表态。
...
要看的不是某一行的具体数字，而是三段占比的移动方向。它的演变揭示了三个事实：(1) 总算力规模 4 年涨 250 倍，年化增长 4×，与 Epoch AI 训练算力 4-5×/年的测算一致，但总算力规模指数同时包含训练 + 推理，意味着推理算力的增长速度与训练算力同量级；(2) 前训练占比从 80% 跌到 35%，但绝对值在涨（35% × 250 = 87.5×，是 2022 年前训练算力的 87.5 倍），不是前训练在缩减，是推理与后训练涨得更快；(3) 推理算力占比的50% 跨越在本章预测中发生在 2027 年——这是本章对「推理主导要到 2027 之后」这条判断 的需求侧承接。

各家对三段占比的口径分歧值得单独说明。McKinsey 在 2024-2025 的 AI Compute Outlook 系列里把后训练算力大部分计入推理算力侧（McKinsey 的口径是任何不是 base model 训练的 compute 都算推理-related），所以 McKinsey 的训推比例是 1:3——训练 25% / 推理 75%。Epoch AI 倾向把后训练算力计入训练侧（任何使用 RL / supervised 微调的 compute 都算训练），所以 Epoch 的训推比例接近 1:1——训练 50% / 推理 50%。两方测算的训推比例差 3 倍，根本不是数据差异，是口径定义差异。把后训练单独拎出来作为第三段之后，两方测算可以收敛到同一张表上——这是 13.4 节论证的核心。

## 13.3 后训练算力的爆发：从 o1 到 RLVR

把后训练算力的增长机制单独拆一节，是因为这是 2024-2026 三段结构最重要的变化点。

**OpenAI o1 / o3 系列的推理时训练扩展**。o1 在 2024-09 发布时，OpenAI 在博客里给出一张关键图——模型性能随训练时计算（train-time compute）和测试时计算（推理时计算 / 推理 compute）两个维度同时 scaling。这张图的工程含义是：以前模型能力的提升只来自训练时投入更多 FLOP，o1 之后模型可以通过在推理时让模型多想一会儿来提升能力。多想一会儿在工程上的实现是 RL-trained chain-of-thought——base model 在 RL 阶段被训练成能生成长思考链，每次回答用户前先生成数千到数十万 token 的思考过程。

这件事对算力消耗的影响是两层：第一层是 RL 阶段本身的算力消耗——把 base model 训练成会长思考需要数月的 RL 训练；第二层是推理时的额外消耗——每次 query 的 token 消耗从 GPT-4o 时代的几百 token 提升到 o1 / o3 时代的几千到数十万 token（业内估算，OpenAI 不直接披露 per-query 推理 token 消耗）。两层叠加，o1 的每次回答的总算力消耗是 GPT-4o 的 30-100 倍。o3（2024-12 预览 / 2025-Q1 正式发布）在 ARC-AGI 基准测试上的高算力配置消耗 \$20+ per task，单次任务的推理成本接近一个工程师 1 小时的工资。
...
| 路径 | 主导公司 | 算力工程 | 后训练 / 总训练比例| 关键时点 |
|---|---|---|---:|---|
| RLHF + Constitutional AI | Anthropic | AI 反馈 + 宪法原则，可规模化 | Claude 2: <10% → Claude 4: 30-50% | 2022-12 论文，2024-2026 工程化 |
| o1-style 推理时训练 | OpenAI | RL-trained chain-of-thought | o1: ~40%| 2024-09 o1，2024-12 o3 |
...
后训练算力爆发带来两层影响。第一层是训推比例的口径必须重新定义——后训练算力在物理上是训练阶段的算力消耗（在训练集群上跑、不直接面向终端用户），但在产业意义上更接近推理算力的前置（后训练优化的是推理时的能力）。McKinsey 把后训练计入推理是从产业意义出发，Epoch 计入训练是从物理意义出发，两者都有道理。第二层是后训练算力的爆发让训练算力 这个变量在 2024-2026 的增速比 Epoch AI 2024-05 测算的 4-5×/年更快——Epoch 的数据库主要追踪前训练算力（base model），后训练算力的增长是 Epoch 数据之外的额外项。

## 13.4 训推比例之争：吵的其实是记账方式

这是 2024-2026 年券商研报里被反复提起的一个争议。问题表述是：未来 5 年 AI 算力支出里，训练算力 vs 推理算力的比例是多少？
...
把口径并排拆清楚之后就会发现，这个问题没有「哪一方对」的答案，只有分情况的答案：
...
三个机制叠起来，推理算力占比跨过 50% 的时点应该在 2027 年中之后，而不是 2026。卖方主流叙事 AI 即推理 在 2024-2025 提前喊出这个判断，是把 5-10 年的渗透曲线压缩成 2 年——这是「训推比例」之争与「推理主导要到 2027 之后」这条判断的核心分歧。

## 13.5 杰文斯效应成立的三个前提

这一节正面回答杰文斯效应还能主导多久。券商的主流说法是「AI 需求无限、杰文斯主导」，把价格跌、需求就涨 当作不需要证明的定律。本章不接受这个跳跃——杰文斯是有边界条件的经济学规律，不是物理定律。

先把杰文斯效应效应的概念在算力上重新校准。19 世纪英国经济学家 William Stanley Jevons 在 1865 年的《The Coal Question》里观察到：蒸汽机效率提升（每磅煤产出更多功率）反而让英国煤炭总消费量上涨。原因是效率提升压低了使用成本（每单位有效功率的煤更便宜），激发出更多新用途（铁路、纺织、钢铁），新需求超过效率节省。这就是杰文斯悖论（Jevons paradox），也叫反弹效应（rebound effect）。
...
但杰文斯效应不是物理定律。第 2 章已经引出过它成立的三个前提，这里把每个条件拆深，给 2024-2026 的实证 + 边界推演。
...
**反面边界**：如果未来某代 AI 算力价格 → 0，但 AI 已经覆盖一切应用（边际应用价值低），杰文斯效应会减弱。在 2026 年中这一时点，AI 应用空间饱和度业内估算约 5-10%——按 Menlo Ventures 2024 报告，2024 年企业 AI 已确认部署用例覆盖率为：code copilots 51%、客服 聊天机器人 31%、企业搜索 28%、数据抽取 27%、会议总结 24%。这些是已确认部署，远没到所有可部署用例都部署完。边界条件 1 在 2024-2026 强成立，2027-2030 仍大概率成立。

### 边界条件 2：算力使用的瓶颈 不在算力本身

杰文斯效应的前提是算力本身是约束——如果算力下游的瓶颈（电力、数据中心、人才、监管）让算力使用受限，杰文斯效应会被瓶颈封顶。

**反面证据**：第 10 章已经展开美国 PJM（Pennsylvania-Jersey-Maryland Interconnection，宾夕法尼亚-新泽西-马里兰州际互联，美国最大区域电力市场运营商）容量市场清算价两年跳涨 11 倍，电力已经成为算力下游的硬约束。变压器交期 2-4 年、电网升级 24 个月、新建数据中心 site 接入 24-36 个月——这些约束都不会因为算力价格下降而消失。如果算力便宜了 100×，但电不够 成为现实，杰文斯效应会卡在电力天花板上。

**当前状态判断**：2024-2026 算力使用的主要瓶颈仍在算力本身——超大规模云厂在 2025-2026 反复说 compute-bound 不是叙事话术，是电力可用之后第二顺位的硬约束（超大规模云厂真实瓶颈是电力 + 算力 双瓶颈，前面数据中心电力章与双瓶颈物理章已展开）。但 2027-2030 电力约束会成为越来越显著的瓶颈。这意味着杰文斯效应在 2024-2026 仍主导，2027-2030 开始受电力天花板部分制约。

边界条件 2 在 18-36 月窗口内不构成对杰文斯效应的硬制约，但在 36 月之外是关键变量。
...
**真正的替代风险**：如果出现算法 / 硬件结构性突破 让 AI 的算力依赖 在某一代发生跳跃下降（如更高效的 attention 机制、神经形态芯片、量子加速），那时杰文斯效应会减弱。这是全书末两章的尾部场景，不在 18-36 月窗口内。
...
未来 18-36 个月（2026-05 至 2029-05 之间），杰文斯效应效应主导算力总需求曲线。具体表达：
...
- 三个边界条件在 18-36 月窗口内均成立或部分成立，没有出现杰文斯效应中断 的迹象。

但这种主导不是永久的。杰文斯效应开始减弱的时点判断在 2030-2035 之间，三个边界条件中至少一个会出现实质性变化。这一判断与后续宏观 TFP 章节、五年之后的尾部场景配合，是全书算力周期长波 判断的一部分。

## 13.6 单 token 价格曲线 vs 推理总量曲线：量价双重运动

把杰文斯效应的实证落到具体数字上。

把 2023-2026 年的单 token 推理价格收束到一张表。这里用 GPT-3.5 / GPT-4 / GPT-4o / Claude 3.5 / DeepSeek R1 / Gemini Pro 系列六个代表性时点：
...
两层曲线同时存在意味着算力需求的弹性不是单一数字——同档替换（用 GPT-4o mini 替代 GPT-3.5）的需求弹性高，能力跃迁（用 o3 / Claude 4 thinking 解决 GPT-4 时代不可能任务）的需求弹性也高。两条曲线叠加，单 token 价格的加权平均 跌幅在 70-90% 之间（取决于市场中同档替换 vs 能力跃迁 的需求结构）。

把价格曲线放在推理总量曲线 旁边对照：

| 时点 | 单 token 价格（同档 input，\$/1M）| 推理总 token 量（trillions / 月）| AI 推理总支出（\$B / 月）|
|---|---:|---:|---:|
...
> 来源：单 token 价格综合 OpenAI / Anthropic / DeepSeek 历史定价。推理总 token 量为业内估算综合 OpenAI 公开访谈（我们每天处理 trillions of tokens）+ Anthropic 年化经常性收入 \$30B 反推 + Google / Microsoft 财报披露的 AI 调用增长率。所有数字为业内估算，不同来源差 30-50%。AI 推理总支出 = 价格 × 总量，业内估算综合反推。
>
> **口径说明（量价算术自验）**：表中 单 token 价格列取的是同档低价 锚点（GPT-4o mini / DeepSeek V4 Flash 一类小模型 input 价），不能直接乘以总 token 量得到总支出——实际市场是混合定价：高档 reasoning 模型（o1 / o3 / Claude Opus 4 thinking，\$15-75/1M）+ 中档（GPT-4o / Claude 3.5 Sonnet，\$3-15/1M）+ 低档（mini / nano / Flash，\$0.14-0.40/1M）同时存在。若全部 5000T tokens 都按 \$0.14/1M 计算，总支出约 \$0.7B/月；按 §13.6 列出的混合市场结构反推加权平均单价约 \$0.40-0.60/1M，与 5000T tokens/月相乘得 \$2-3B/月（章内取 \$2B/月是偏保守的中位数）。读者按自己估算的加权平均价格调整即可自验。

**量价双重运动的核心结构**：3 年里同档 单 token 价格跌 99.5%（200×↓），但推理总 token 量涨 1000×（从 5T 到 5000T），推理总支出涨 13×（从 \$150M/月到 \$2B/月）。这是杰文斯效应效应在算力上的实证——价格暴跌但总支出大幅上涨。

把这件事翻译成对算力供应商的影响：单 token 算力消耗成本（从英伟达 / 台积电视角）跌幅没有 99.5% 这么大，业内估算约跌 70-80%（H100 → H200 → B200 → GB200 的硬件吞吐量提升 + KV Cache 等软件优化）。模型公司的 单 token 毛利率 从 GPT-4 时代的 -200%（OpenAI 早期亏损）演变到 2026 年的 +10-30%——单 token 价格跌幅与单 token 成本跌幅的剪刀差是 OpenAI / Anthropic 2025-2026 年化经常性收入与毛利率同时提升的核心机制。

**两类补贴定价 的口径说明**：表中部分价位（如 DeepSeek R1 首发 cache hit \$0.14、GPT-4o mini \$0.15）业内估算属于低于成本的市场份额定价。OpenAI 与 Anthropic 在主战场（GPT-4 / Claude 3.5+）的实际毛利率为正，但在 GPT-4o mini / nano 等小模型档的定价可能是按英伟达 H100 成本反推后毛利接近 0（综合 SemiAnalysis 2025 + The Information 2025-06 报道）。Cursor / Replit / Devin 等编码助手 的定价是另一层补贴——按 token 成本反推，每次 agent 任务实际消耗的 API 成本可能超过 Cursor 收取的 \$20 / 月订阅费的 50%-200%，剩余靠风投资金补贴。这一层补贴的可持续性是第 18 章（模型层与循环交易）的主题。

但补贴定价不影响杰文斯效应本身的实证——即使把补贴部分剔除，单 token 价格 3 年仍跌 70-90%（按英伟达 H100 硬件吞吐量提升测算的非补贴价格曲线），推理总 token 量 3 年仍涨 100-500×，总支出仍涨 5-10×。补贴部分让价格跌得比硬件跌得快，但杰文斯效应的方向判断不变。

## 13.7 Brynjolfsson J 曲线：企业 IT 投资的生产率显化时滞

把视角从供应端切到需求端。前面几节给的单 token 价格曲线和推理总量曲线，都是从模型公司的视角看市场。但杰文斯效应的真正驱动力来自需求端——企业用户用 AI 做什么、付多少钱、什么时候付。Brynjolfsson J 曲线是回答这个问题的理论框架。

Erik Brynjolfsson 与 Lorin Hitt 在 1990s-2000s 的学术系列里提出 J 曲线观察：企业大规模 IT 投资之后，全要素生产率（TFP）不会立刻上升，而是有 5-10 年的滞后期，期间生产率甚至下降（投资 → 组织摩擦 → 流程调整），然后才进入显化期。Brynjolfsson, Rock, Syverson 在 2018 / 2021 的论文里把这个框架应用到通用目的技术（GPT，General-Purpose Technology），给出 GPT-style 技术的生产率 J 曲线 三阶段：
...
> 来源：J 曲线框架来自 Brynjolfsson, Hitt 2000 Beyond Computation *JEP* 14(4) / Brynjolfsson, Rock, Syverson 2018 NBER WP 25148 与 2021 *AEJ:Macro* 13(1)。AI 周期时间映射来自第 2 章 §6 + 第 24 章 / 第 25 章主讲。BLS TFP 数据来自美国劳工统计局多因素生产率数据库。
...
把 J 曲线与杰文斯效应连起来看：J 曲线投资期的核心特征是企业大规模买进 AI 服务，但生产率显化滞后 ——这意味着企业 AI 预算的扩张早于生产率回报，扩张的资金来源是企业 IT 预算的再分配 + 风险投资 + 创新预算。Menlo Ventures 2024 报告显示：60% 企业 AI 投资来自创新预算（experimental），40% 来自永久预算（其中 58% 是从存量预算再分配）。这意味着 2024 年企业 AI 支出 \$138 亿里，永久预算实际新增只有 ~\$23 亿——大部分还是试验性预算。
...
这件事对「杰文斯主导还是预算约束」这个问题的影响是：**预算约束在 18-36 月窗口内不构成杰文斯效应的硬制约**。理由：(1) 企业 AI 预算的硬天花板 是企业 IT 总预算的 ~5-15%，2026 年企业 AI 预算占 IT 预算 ~5-8%（业内估算综合 Menlo + Gartner 2024-2026），仍有 1-2 倍的扩张空间；(2) J 曲线投资期的特征是预算扩张早于生产率回报，企业 CFO 在投资期通常不会卡 AI 预算（CFO 的逻辑是先抢占应用先机）；(3) 风险投资 + 超大规模云厂内部预算（不依赖企业付费）在 2024-2026 提供大量补贴定价，让最终 AI 应用的用户付费价 远低于真实算力成本。
...
J 曲线还有一层含义：「推理算力跨过 50% 要到 2027 之后」这个预测，正好对应 J 曲线投资期到调整期的过渡。投资期推理算力的增长是试用驱动 ——企业部署很多 PoC 但单个 PoC 的算力消耗有限；调整期推理算力的增长是生产驱动 ——企业把成功 PoC 推到全员 / 全流程的生产部署，单个企业的推理算力消耗可能跳升 10-100×。J 曲线的阶段切换与推理算力的 50% 阈值跨越在时间上同步——这不是巧合，是同一个机制（企业 AI 渗透从试用到生产）的两个表象。

## 13.8 三套需求场景：2026-2030 三段算力分配

把前面 §13.2-§13.7 的判断收束到 2026-2030 的情景模型。三套场景：base / bull / bear。

**中性场景：杰文斯效应持续，J 曲线进入调整期早段**。这是主场景。
...
> 关键假设：杰文斯效应在 2026-2030 持续（三个边界条件均成立或部分成立）；J 曲线 2027 进入调整期早段，企业 AI 预算进入永久预算扩张阶段；前训练算力绝对值仍在增长但增速从 4-5×/年降到 2-3×/年；推理算力 2028 跨过 50% 阈值。

**乐观场景：杰文斯加速 + Agent 量产爆发 + J 曲线显化期提前**。
...
> 关键假设：AI agent 在 2027-2028 进入大规模生产部署，单次 agent 任务 token 量 10-100× 聊天机器人；企业 AI 采纳率从 2026 的 12% 跳升到 2028 的 40%；J 曲线显化期提前到 2028（vs base 的 2032）；推理算力 2027 跨过 50%；总算力规模 4 年涨 20×。

**悲观场景：电力约束 + 客户集中度反身性 + 模型公司补贴退潮**。
...
> 关键假设：电力约束在 2027-2028 成为硬天花板；客户集中度反身性让超大规模云厂资本支出在 2027-2028 进入紧缩周期；模型公司补贴定价退潮，单 token 价格止跌甚至上行；推理算力增长被预算约束 + 电力约束封顶；前训练 / 后训练算力占比上行（不是绝对值上行，是推理增长缓慢导致占比相对上行）；总算力规模 4 年仅涨 1.9×。电力 + 客户集中度的展开见第 14 章，模型公司补贴定价的展开见第 18 章。

三套场景的算力规模差异在 2030 年达到 10×（bull 20× vs bear 1.9×）。这个差异不是建模灵敏度的问题，是三套场景对杰文斯效应 vs 预算约束 的不同判断。把三套场景的核心差异提炼出来：
...
> 来源：三套场景为本书模型推演，关键假设逐行标注。所有数字为业内估算，单点估值不应被读为准确预测。

三套场景的概率分配是需求侧的判断：**中性场景 60%、乐观场景 15%、悲观场景 25%**。base 与 bull 合计 75% 是杰文斯主导，bear 25% 是预算约束抬头。这个概率分配的核心理由：

(1) 杰文斯效应的三个边界条件在 18-36 月窗口内均成立或部分成立（§13.5），悲观场景的边界突破 需要电力 + 客户集中度 + 补贴退潮三件事同步发生，概率 < 50%（业内估算）；

(2) 乐观场景需要 智能体量产提前 + J 曲线显化期提前，两件事在 2027 同步发生的概率 < 30%（业内估算综合 J 曲线历史时滞 + agent 应用层成熟度）；
...
## 13.9 什么信号出现，就算这个判断错了

「推理算力要到 2027 之后才主导」这个判断，上一章从供给侧给过条件，这一节从需求侧把它钉死——2027 之前训练算力受 HBM / CoWoS 双瓶颈封顶 + Hopper 推理算力释放释放出有效推理供给。本章给需求侧条件——2027 之前训练算力仍由前沿模型训练 + 后训练范式驱动 + 推理算力的指数级爆发 需要等 J 曲线进入调整期。

四个信号，每个都配基线、阈值、时间窗和监测来源：

| 信号 | 基线（2026-05）| 阈值 | 时间窗口 | 监测来源 | 反向条件 |
|---|---|---|---|---|---|
| 1. 推理算力占比 | ~40%| 跨过 50%，连续 4 个季度 | 2027-Q2 至 2028-Q2 | Epoch AI 算力数据库季度更新 + 英伟达 / TPU / Trainium / MTIA 装机数据按训练池 / 推理池拆分 | 2026 年提前跨过 50%（「推理主导要到 2027 之后」这条判断 错，应已经成立）|
| 2. ChatGPT / Claude 等 聊天机器人 总 token 月度量 | ~5000T tokens/月| 跨过 100,000T tokens/月（20× 增长）| 2027-Q4 至 2028-Q4 | OpenAI / Anthropic 公开年报 + 第三方应用监测（Similarweb / The Information）| 2027 提前跨过（推理爆发提前）|
...
(a) **2027-Q2 至 2028-Q2 期间，信号 1（推理算力占比跨过 50%）必须触发**——否则「推理主导要到 2027 之后」这个判断的需求侧承接错。

(b) **2026 年底前，信号 1、2、3 任何一个提前触发**——这是判断错的另一种形式（推理主导应该已经成立，而非2027 之后才成立）。

(c) **信号 4（前沿模型前训练算力增速降到 2×/年）在 2027-2029 触发**——这是前训练放缓 的间接证据，触发或不触发都可以独立判断方向但不否定主信号。

(d) **保守证伪条件**：如果 2028 年底信号 1 仍未触发（推理算力占比仍 < 50%），同时信号 2、3 也未触发，「推理主导要到 2027 之后」这条判断 整体作废。

为什么主信号选「推理算力占比跨过 50%」，而不是「推理算力绝对值翻倍」？因为绝对值翻倍在中性和乐观场景下都会发生（2026 → 2027 即使按 base 也涨 1.8×，推理算力绝对值会跳升），不能区分主导切换 vs 整体扩张。占比跨过 50% 这件事必须同时满足推理涨得快 + 训练 + 后训练涨得慢，是真正能区分「推理主导要到 2027 之后」这条判断 与卖方主流叙事的信号。
...
还有一层意义：如果这个判断错了，全书围绕「2027 拐点」的叙事就需要重新讨论。前一章供给侧的反共识承诺与本章需求侧的反共识承诺是配对的——两者其中之一证伪，2027 拐点的判断会向另一个方向修正。
...
第三，训推比例之争的核心分歧是后训练算力怎么归类——McKinsey 计入推理（1:3）vs Epoch AI 计入训练（1:1），事实层没有矛盾，分歧全在口径定义。三段结构让两方收敛到同一张表。

第四，杰文斯效应与预算约束之争，判断明确——18-36 月窗口内杰文斯效应主导。三个边界条件（使用门槛低 + 应用未饱和 / 算力非下游瓶颈 / 互补关系占主导）在窗口内均成立或部分成立。

第五，「推理算力主导要到 2027 之后」在需求侧的承接——前训练算力到 2026 仍在加速增长（Epoch 4-5×/年）+ J 曲线投资期 vs 调整期切换在 2027-2028 + 推理算力占比50% 阈值跨越 时点是 2027-Q2 至 2028-Q2。

第六，把话说死——四个可证伪信号（推理算力占比 / 聊天机器人总 token 量 / 企业 AI 采纳率 / 前训练算力增速）任何一个在 2026 年底前提前触发或 2028 年底前主信号未触发，「推理主导要到 2027 之后」这条判断 整体作废。

三段结构、三个前提、三套需求场景、四个证伪信号——两年之后回头打分，这一章的命门在主信号推理算力占比跨过 50% 是否在 2027-Q2 至 2028-Q2 期间触发。
